...
class AllFloatButton: UIButton {
    ...
    override init(frame: CGRect) {
        ...
        //音乐
        musicList = Bundle.main.urls(forResourcesWithExtension: ".mp3", subdirectory: nil)
        player = try? AVAudioPlayer(contentsOf: musicList![current])
        playlayer?.position = CGPoint(x: 0, y: 0)
        player?.play()
    }
